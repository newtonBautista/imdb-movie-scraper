...
MAX_THREADS = 15
movies_list = []  # Lista temporária para armazenar os filmes
        ...
        if page_section is not None:
            ...
            if len(divs) > 1:
                ...
                if all([title, date, rating, plot_text]):
                    ...
                    movies_list.append([title, date, rating, plot_text])
...
def save_movies():
    """ Ordena os filmes por nota e salva no CSV """
    global movies_list
    # Converte as notas para float antes de ordenar
    movies_list.sort(key=lambda x: float(x[2].split('/')[0]), reverse=True)

    with open('movies.csv', mode='w', newline='', encoding='utf-8') as file:
        movie_writer = csv.writer(file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
        movie_writer.writerow(["Título", "Ano", "Nota", "Sinopse"])  # Cabeçalho
        movie_writer.writerows(movies_list)  # Escreve os filmes ordenados
...
def main():
    ...
    extract_movies(soup)
    save_movies()  # Salva os filmes após coletar todos os dados
